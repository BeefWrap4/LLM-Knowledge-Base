...
| **成本模型** | CPU核数 x 时间 | GPU型号 x 显存 x 时间（差异可达50倍） | Spot 实例、自动缩容、模型量化 |

### 24.1.3 大模型部署架构全景图

下面展示一个生产级大模型推理服务的完整云原生部署架构：
...
graph TB
    ...
    subgraph "入口层"
        ...
        GW["API 网关<br/>Envoy/Kong/APISIX"]
    end
    ...
    subgraph "Kubernetes 集群"
        ...
        subgraph "推理节点池 (GPU)"
            GPU1["Pod: vLLM-Qwen3-72B<br/>GPU: A100-80G x4"]
            GPU2["Pod: vLLM-Qwen3-72B<br/>GPU: A100-80G x4"]
            GPU3["Pod: vLLM-Qwen3-7B<br/>GPU: A10-24G x1"]
        end

        subgraph "嵌入节点池 (GPU)"
            EMB1["Pod: Embedding-Server<br/>GPU: T4-16G x1"]
            EMB2["Pod: Reranker-Server<br/>GPU: T4-16G x1"]
        end

        subgraph "计算节点池 (CPU)"
            CPU1["Pod: API-Backend"]
            CPU2["Pod: 文档处理"]
            CPU3["Pod: 权限/审计"]
        end
    end
    ...
    subgraph "基础设施"
        PVC["持久化存储<br/>模型仓库（PVC/Ceph）"]
        MON["可观测性栈<br/>Prometheus + Grafana"]
    ...
    U3 --> LB
    LB --> GW
    GW --> |"chat/completions"| GPU1
    GW --> |"chat/completions"| GPU2
    GW --> |"chat/completions (light)"| GPU3
    GW --> |"embedding"| EMB1
    GW --> |"rerank"| EMB2
    GW --> |"metadata/auth"| CPU1
    ...
    style GPU1 fill:#ffebee,stroke:#d32f2f
    style GPU2 fill:#ffebee,stroke:#d32f2f
    style GPU3 fill:#ffebee,stroke:#d32f2f
    style EMB1 fill:#e8eaf6,stroke:#3f51b5
...
| **序列化速度** | 慢（JSON parse/stringify） | 快（Protobuf 编解码，约 3-10x） |
| **数据体积** | 大（文本+冗余字段名） | 小（二进制紧凑编码，约 2-5x 压缩） |
...
### 24.4.4 性能对比（延迟/吞吐/序列化效率）

**基准测试条件**：Qwen2.5-7B-Instruct, A10 GPU, 输入 512 tokens, 输出 256 tokens, 并发 32。

| 指标 | REST (JSON) | gRPC (Protobuf) | 提升 |
|------|------------|----------------|------|
| **平均延迟（P50）** | 1,250 ms | 1,180 ms | -5.6% |
| **尾延迟（P99）** | 3,800 ms | 2,950 ms | **-22.4%** |
| **吞吐（req/s）** | 18.2 | 22.7 | **+24.7%** |
| **网络传输体积/请求** | 1,840 bytes | 620 bytes | **-66.3%** |
| **序列化/反序列化耗时** | 3.2 ms | 0.8 ms | **-75.0%** |
| **HTTP/2 多路复用增益** | N/A | 连接复用减少 80% TCP 开销 | — |
...
| **金丝雀发布** | 5% 流量到新版，逐步加大 | 降低新模型上线风险 | 权重路由 |
...
### 24.6.5.2 PD-Disaggregation on NVL (Prefill/Decode 分离)
...
| 指标 | 传统同 Pod | PD-Disaggregation | 提升 |
|------|-----------|------------------|------|
| TTFT (P50) | 380ms | 165ms | **-56.6%** |
| TPOT (P99) | 95ms | 62ms | **-34.7%** |
| 整体吞吐 (req/s) | 18.4 | 31.2 | **+69.6%** |
| Prefill GPU 利用率 | 45% (Decode 阻塞) | 89% | +44pp |
...
### 24.6.5.4 FP4 / NVFP4 / MXFP4 量化阶梯

| 量化格式 | 每参数位宽 | 硬件支持 | 精度损失 | 适用场景 |
|---------|----------|---------|---------|---------|
| **FP16/BF16** | 16 | 全部 | 基准 | 训练、对精度敏感 |
| **INT8 (W8A8)** | 8 | NVIDIA A100+ | ~0.1% | 通用推理 |
| **FP8 (E4M3 / E5M2)** | 8 | H100+ / B200 | ~0.3% | NVIDIA 新代 |
| **AWQ / GPTQ (INT4)** | 4 | 全部 | ~0.8% | 显存受限 |
| **NVFP4 (E2M1 + 块缩放)** | 4 | B200 (Blackwell) | ~0.5% | Blackwell 推荐 |
| **MXFP4 (OCP 8×E2M1)** | 4 | B200 / TPU v6 / Gaudi 3 | ~0.6% | 跨硬件统一 |
| **GGUF Q4_K / Q2_K** | 4–2 | CPU / Apple Silicon | 1–3% | 端侧 / 边缘 |

**vLLM NVFP4 部署示例：**

```bash
# B200 节点上以 NVFP4 加载 DeepSeek-V3-671B
python -m vllm.entrypoints.openai.api_server \
  --model /models/DeepSeek-V3-671B-NVFP4 \
  --quantization nvfp4 \
  --kv-cache-dtype fp8 \
...
### 24.6.5.5 Speculative Decoding 作为部署期旋钮

Speculative Decoding（推测解码）从"训练期技巧"演变为**部署期的关键性能旋钮**。它在不改变模型输出分布的前提下，通过小模型"猜测"+ 大模型"批量验证"加速生成。

| 方法 | 草稿模型 | 加速比 | 显存开销 | K8s 部署形态 |
|------|---------|--------|---------|-------------|
| **EAGLE-3** | 1 层 Transformer | 2.0–2.8× | +10% | 同一 Pod 加载 draft |
| **Medusa** | 多个 MLP 头 | 1.5–2.2× | +5% | 同模型权重 |
| **n-gram** | 无（纯文本匹配） | 1.3–1.7× | 0 | Tokenizer 端 |
| **Draft Model** | 独立 0.5B–3B 模型 | 2.5–3.5× | +20% | 独立 Sidecar Pod |
| **Self-Speculative** | 同一模型浅层 | 1.4–1.8× | 0 | 无额外资源 |
...
### 24.6.5.6 硬件可移植性：TPU / Gaudi / Ascend / Apple Silicon
...
### 24.6.5.7 Ollama Cloud + ollama launch (2026 新形态)
...
client_cloud = ollama.Client(
    ...
    headers={"Authorization": "Bearer ${OLLAMA_CLOUD_TOKEN}"}
)
...
### 24.6.5.8 Anthropic Managed Agents — 自托管沙箱与 MCP Tunnels

Anthropic 在 2026 年推出 **Managed Agents** 服务形态，核心特性：

1. **自托管沙箱（Self-hosted Sandboxes）**：在客户自己的 K8s 集群中运行 Agent 执行环境
2. **MCP Tunnels**：Anthropic 托管控制平面通过加密隧道连接客户集群中的沙箱
3. **零数据外泄**：代码执行、文件操作全部在客户 VPC 内

**架构示意：**

```mermaid
flowchart TB
    subgraph Anthropic["Anthropic 托管控制平面 (AWS us-east-1)"]
        CTRL["Agent 协调器<br/>Claude API Gateway"]
        MCP_SVR["MCP Server Registry"]
        OBS["可观测性<br/>LangSmith-style"]
    end

    subgraph CustomerVPC["客户 VPC (私有网络)"]
        subgraph K8s["客户 K8s 集群"]
            subgraph Sandboxes["Agent 沙箱命名空间"]
                SB1["Sandbox Pod 1<br/>Claude Agent Runtime<br/>+ 工具执行"]
                SB2["Sandbox Pod 2<br/>Claude Agent Runtime"]
                SB3["Sandbox Pod 3<br/>Claude Agent Runtime"]
            end
            Tunnel["mcp-tunnel DaemonSet<br/>eBPF + WireGuard"]
            MCPS["自托管 MCP Servers<br/>客户内部工具/数据源"]
        end
    end

    CTRL <-->|"加密隧道<br/>mTLS + WireGuard"| Tunnel
    MCP_SVR <-->|"服务发现"| Tunnel
    OBS <-->|"仅元数据<br/>不含 prompt/数据"| Tunnel
    Tunnel --> SB1
    Tunnel --> SB2
    Tunnel --> SB3
    SB1 -->|"工具调用"| MCPS
    ...
    style Anthropic fill:#d7ccc8,stroke:#3e2723
    style CustomerVPC fill:#e8f5e9,stroke:#1b5e20
    style Tunnel fill:#fff59d,stroke:#f57f17
```

**Anthropic Managed Agents Helm Values 示例：**
...
anthropicManagedAgents:
  ...
  version: "1.4.0"
  cluster:
    name: "customer-prod-1"
    region: "us-west-2"
  tunnel:
    mode: wireguard
    endpoint: "wg://tunnel.anthropic.com:51820"
    persistentKeepalive: 25
    auth:
      tokenSecret: anthropic-connection-token
    egressPolicy:
      denyAll: true
      allow:
  ...
  sandboxes:
    runtime:
      image: "anthropic/agent-runtime:1.4.0"
      resources:
        requests:
          cpu: "1"
          memory: "4Gi"
        ...
        - from:
          ...
          ports:
            - protocol: TCP
              port: 7000
  mcpServers:
    - name: customer-internal-db
      transport: stdio
      image: "customer/mcp-postgres:1.0"
      env:
        DATABASE_URL: "postgresql://readonly@db.internal:5432/agent_ctx"
    - name: customer-jira
      transport: http
  ...
  observability:
    exportTraces: true
    traceEndpoint: "https://api.anthropic.com/v1/traces"
    redactPrompts: true           # 不外发 prompt 内容
    redactToolInputs: true
    keepLocalAuditLog: true
    auditLogPath: "/var/log/anthropic-agent/audit.jsonl"
```

**部署流程：**

```bash
# 1. 在 Anthropic Console 创建 Cluster Enrollment Token
# 2. 在客户集群中安装 Helm Chart
helm repo add anthropic https://charts.anthropic.com
helm install anthropic-agents anthropic/anthropic-managed-agents \
  --namespace anthropic-system --create-namespace \
  --set cluster.name="customer-prod-1" \
  --set tunnel.auth.tokenSecret="anthropic-connection-token" \
  --set sandboxes.isolation.runtimeClassName="gvisor"

# 3. 验证隧道建立
kubectl logs -n anthropic-system deploy/mcp-tunnel --tail 50
# 期望看到: "Tunnel established, peer endpoint: tunnel.anthropic.com"
```

**与传统 OpenAI/Anthropic API 的关键区别：**

| 维度 | 公有云 API | Managed Agents 自托管 |
|------|-----------|---------------------|
| 代码执行位置 | Anthropic VPC | 客户 VPC |
| 工具/MCP 部署 | 客户公网暴露 | 客户内网 |
| 数据出域 | Prompt/工具结果过公网 | 仅元数据（统计/审计） |
| 网络要求 | 出站 HTTPS 即可 | 出站 UDP 51820 (WireGuard) |
| 合规等级 | SOC2 + DPA | SOC2 + 客户 VPC 完全隔离 |
| 计费模式 | 按 Token | 按 Agent 小时数 |

### 24.6.5.9 推理引擎选型决策速查（2026）

| 你的场景 | 推荐引擎 | 关键配置 | 成本基线（$/M tokens） |
|---------|---------|---------|---------------------|
| 通用 7B–70B 生产 | vLLM v1.0 | TP=4, FP8, Prefix Cache | 0.30 |
| 长上下文/Agent | SGLang + PD | EP=8, NVFP4, EAGLE-3 | 0.55 |
| 极致 NVIDIA 性能 | TensorRT-LLM | TP=8, FP4, In-flight batching | 0.18 |
| 端侧/边缘 | MLC-LLM | WebGPU/Metal, INT4 | 自托管 |
| CPU/小模型 | llama.cpp | GGUF Q4_K_M | 0.05 |
| TPU 环境 | vLLM-TPU | MXFP4, TP=4 | 0.22 |
| Gaudi 性价比 | vLLM-HPU | INT8, TP=8 | 0.12 |
| Ascend 国产化 | MindIE + vLLM | W8A8, EP=16 | 0.15 |
| 快速原型 | Ollama + ollama launch | GGUF Q4, --gpu all | 0.08 |
| 合规/数据不出域 | Anthropic Managed Agents | mcp-tunnel + gvisor | 按 Agent 小时 |
...
| **GPU Cloud (CoreWeave/Lambda)** | 独立 | 裸 GPU 实例，价格优惠 50-70% | 按实例小时 | 低 |
...
大模型推理的成本是传统 Web 服务的 10-100 倍，成本优化是工程化的核心能力。

| 策略 | 原理 | 成本节省 | 实现复杂度 | 适用场景 |
|------|------|---------|-----------|---------|
| **Spot / 抢占实例** | 使用云厂商折扣实例 | 60-80% | 中 | 离线批量推理 |
| **模型缓存** | Prefix Cache 复用 KV Cache | 30-50% Token 成本 | 低 | 有公共前缀的对话 |
| **自动缩容** | 空闲时自动缩到 minReplicas | 40-60% | 中 | 流量有昼夜规律 |
| **模型量化** | INT8/AWQ/FP8 降低显存 | 30-70% 显存 | 中 | 精度要求不极端的场景 |
| **请求合并** | 合并小请求批量推理 | 20-40% 吞吐提升 | 中 | 后端 API 调用 |
| **多模型分级** | 简单问题用小模型 | 50-80% per 请求 | 高 | 有明确难度分级的场景 |
| **预留实例** | 预付 1-3 年 | 30-50% | 低 | 稳定基线流量 |
...
大模型推理成本通常是传统服务的 10-100 倍，成本优化是面试中的高频考点。

**核心策略（按性价比排序）：**

1. **Spot/抢占实例（60-80% 节省）**：适合离线批量和可中断的推理任务。使用 K8s 的 `priorityClassName` 将 Spot Pod 设为低优先级，当 Spot 回收时自动切换到 On-Demand。

2. **Prefix Cache（30-50% 节省）**：vLLM/TGI 的 Automatic Prefix Caching (APC) 自动缓存并复用公共前缀（如 System Prompt）的 KV Cache，大幅减少重复计算。

3. **模型量化（30-70% 显存节省）**：使用 AWQ、GPTQ、FP8 量化将模型精度从 FP16 降到 INT4/INT8，在几乎不影响质量的情况下，同一块 GPU 可以服务更多请求。
...
7. **预留实例（30-50% 节省）**：对于稳定的基线流量（如工作时间的对话量），购买 1-3 年预留实例可大幅降低单价。
...
| 模型推理部署 | vLLM/SGLang/TRT-LLM 三大引擎；Prefill/Decode 分离（PD-Disaggregation）降低 TTFT 56%；MoE 专家并行需 Volcano 调度同 NVLink Domain |
| gRPC 通信 | HTTP/2 多路复用 + Protobuf 二进制（带宽 -66%）；支持 Server Streaming 流式推理；适合微服务间高吞吐场景，需 gRPC-Web 兼容浏览器 |
...
| 多云与成本 | Spot GPU 实例可降本 60-70%；模型量化（NVFP4/INT8 AWQ/GGUF）减半显存；HPA 提前量预留缓冲避免冷启动 |
...
```mermaid
flowchart TB
    subgraph "K8s 大模型推理集群架构"
        direction TB

        LB["负载均衡器<br/>Ingress / Nginx"]

        subgraph Control["控制平面"]
            API["API Server"]
            SCHED["Scheduler<br/>(含 GPU 感知调度)"]
            CM["Controller Manager"]
        end

        subgraph GPU_Node1["GPU 节点组 A (A100×8)"]
            direction TB
            K1["kubelet"]
            D1["NVIDIA Device Plugin"]
            P1_1["Pod: vLLM-Qwen-72B<br/>GPU: 0,1,2,3<br/>显存: 4×80GB"]
            P1_2["Pod: TGI-Llama-8B<br/>GPU: 4,5<br/>显存: 2×80GB"]
            P1_3["Pod: Embedding-Svc<br/>GPU: 6,7<br/>显存: 2×80GB"]
        end

        subgraph GPU_Node2["GPU 节点组 B (H200×8)"]
            direction TB
            K2["kubelet"]
            D2["NVIDIA Device Plugin"]
            P2_1["Pod: vLLM-DeepSeek-R1<br/>GPU: 0,1,2,3,4,5,6,7<br/>显存: 8×141GB"]
        end

        subgraph CPU_Node["CPU 节点组"]
            direction TB
            P3_1["Pod: API Gateway"]
            P3_2["Pod: Redis Cache"]
            P3_3["Pod: Prometheus"]
            P3_4["Pod: Grafana"]
        end
    ...
    style LB fill:#4A6FA5,color:#fff
    style GPU_Node1 fill:#e8f5e9,stroke:#388e3c
    style GPU_Node2 fill:#e8f5e9,stroke:#388e3c
    style CPU_Node fill:#e3f2fd,stroke:#1976d2
    style P2_1 fill:#ffcdd2,stroke:#d32f2f
```
...
```mermaid
flowchart LR
    Client["客户端请求"] --> Gateway["模型网关<br/>流量管理"]

    Gateway --> Router{"路由决策<br/>模型/版本选择"}

    Router -->|"v2 金丝雀 10%"| ModelV2["Qwen-72B v2<br/>(新版本)"]
    Router -->|"v1 稳定 90%"| ModelV1["Qwen-72B v1<br/>(稳定版)"]

    Router -->|"复杂推理"| LargeM["DeepSeek-R1<br/>671B MoE"]
    Router -->|"简单问答"| SmallM["Qwen-7B<br/>轻量模型"]

    Gateway --> Queue["请求队列<br/>背压处理"]
    Queue --> RateLimit["限流器<br/>Token Bucket"]

    Gateway --> Monitor["监控上报<br/>延迟/吞吐/错误"]
    ...
    Monitor --> Alert["告警<br/>>500ms 延迟<br/>P50/P95/P99"]

    style Gateway fill:#4A6FA5,color:#fff
    style Router fill:#ff9800,color:#fff
    style ModelV2 fill:#ffcdd2,stroke:#d32f2f
    style ModelV1 fill:#c8e6c9,stroke:#388e3c
    style LargeM fill:#e1bee7,stroke:#7b1fa2
    style SmallM fill:#b3e5fc,stroke:#0288d1
