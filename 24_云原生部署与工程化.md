...
> [!abstract] 本章导航
> **定位**：把模型应用封装为可交付服务，连接容器、编排、网关和持续交付。
>
> **先修**：[[09_Web开发与FastAPI]]、[[19_分布式训练系统]]、[[20_LLMOps与模型可观测性]]。
>
> **学习目标**：
> - 容器化模型服务并解释运行时资源边界。
...
> - 验证 CI/CD、可观测性和供应链安全。
>
> **建议路径**：云原生概述 → Docker 容器化模型服务 → Kubernetes 编排与弹性扩缩 → … → 多云部署策略。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch24_cloudnative/`。
...
>
> **版本与范围**：新增 NVIDIA Container Toolkit v2.0、Kubernetes GPU 调度增强（DRA）、vLLM v1.0 生产级部署最佳实践、gRPC-Web 流式推理方案、GitHub Actions 模型评估门禁、多云成本优化与 Spot GPU 策略等最新内容。
...
1. 你能否容器化模型服务并解释运行时资源边界？
2. 你能否设计具备扩缩容、流量治理和故障恢复的部署架构？
3. 你能否验证 CI/CD、可观测性和供应链安全？

## 🧪 配套代码与验收

配套目录：`code/ch24_cloudnative/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier gpu --chapter ch24 --parallel 1 --timeout 180
```
...
模型冷启动是指推理 Pod 从启动到能处理请求的时间过长；实际时长取决于权重大小、存储与网络吞吐、反序列化、显存加载和运行时初始化。

**解决方案：**

| 方案 | 原理 | 效果 | 代价 |
|------|------|------|------|
| **预热池（Warm Pool）** | 维持经过容量测算的空闲 Pod | 降低扩容等待，幅度需压测 | GPU 空转成本 |
| **模型缓存持久化** | 将模型缓存在共享存储（Ceph/NFS），避免重复下载 | 减少网络IO | 存储成本 |
| **Lazy Model Loading** | 延迟加载可拆分的模型组件 | 可能降低首阶段启动时间，需按运行时实测 | 实现复杂，且并非所有模型都可拆分 |
| **P2P 模型分发** | 通过 Dragonfly/Kraken 实现 P2P 镜像和模型分发 | 集群级加速 | 节点带宽开销 |
| **HPA 提前扩容** | 基于历史流量和启动时长在峰值前扩容 | 减少请求命中冷实例的概率 | 预测误差可能浪费资源 |
| **SageMaker/Vertex AI** | 使用托管服务的预热和模型缓存能力 | 降低部分平台运维工作 | 成本、能力边界和供应商依赖 |
