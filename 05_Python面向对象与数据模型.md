...
topic_id: python-oop-data-model
difficulty: 中高
...
tags:
  ...
  - 面试教程
---
# 第 5 章 Python 面向对象与数据模型 ⭐⭐⭐⭐
> [!abstract] 本章导航
> **定位**：第一部分 Python 与后端工程基础中的第 5 章；围绕“Python 面向对象与数据模型”建立单一、可追踪的知识主线。
>
> **先修**：[[04_Python迭代协议与函数式编程|第 4 章 Python 迭代协议与函数式编程]]。
>
> **学习目标**：
> - 解释 类与对象基础 ⭐⭐⭐⭐ 的核心问题、机制与适用边界。
> - 实现或评估 面向对象三大特性 ⭐⭐⭐⭐⭐ 的最小闭环。
> - 使用可复现证据诊断 多继承与 C3 线性化算法 ⭐⭐⭐⭐⭐ 的工程取舍与失败模式。
>
> **建议路径**：类与对象基础 ⭐⭐⭐⭐ → 面向对象三大特性 ⭐⭐⭐⭐⭐ → 多继承与 C3 线性化算法 ⭐⭐⭐⭐⭐ → `__new__` vs `__init__` ⭐⭐⭐⭐⭐ → 魔术方法大全 ⭐⭐⭐⭐ → 描述符协议 ⭐⭐⭐ → 生产边界与面试表达。
>
> **配套代码**：`code/ch05_oop_data_model/`。

本章先回答“类与对象基础 ⭐⭐⭐⭐”为什么成立，再沿着机制、实现、评估和边界逐步展开。阅读时先建立因果链，再运行或推演示例，最后用章末自测检查能否脱离原文复述。
## 5.1 类与对象基础 ⭐⭐⭐⭐
...
### 5.5.2 `__getattr__` vs `__getattribute__` ⭐⭐⭐⭐
...
- 类与对象基础 ⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- 面向对象三大特性 ⭐⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- 多继承与 C3 线性化算法 ⭐⭐⭐⭐⭐：能够说清问题、机制、证据与边界。

## ✅ 自测与练习

1. 不看正文，解释“类与对象基础 ⭐⭐⭐⭐”解决什么问题，并给出一个不适用场景。
2. 为“面向对象三大特性 ⭐⭐⭐⭐⭐”设计一个最小可复现实验，明确输入、指标和通过条件。
3. 比较“多继承与 C3 线性化算法 ⭐⭐⭐⭐⭐”的至少两种方案，说明质量、成本、延迟或风险取舍。

## 🧪 配套代码与验收

- `code/ch05_oop_data_model/`

```powershell
python code/scripts/run_all_examples.py --chapter ch05 --tier core
```
...
| 主题 | 回答主线 |
|---|---|
| 类与对象基础 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 面向对象三大特性 ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 多继承与 C3 线性化算法 ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| `__new__` vs `__init__` ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 魔术方法大全 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
