...
# 第 29 章 大模型数据工程 ⭐⭐⭐⭐
> [!abstract] 本章导航
> **定位**：第五部分 数据、训练、对齐、评估与安全中的第 29 章；围绕“大模型数据工程”建立单一、可追踪的知识主线。
>
> **先修**：[[28_ComputerUse与GUIAgent|第 28 章 Computer Use 与 GUI Agent]]。
>
> **学习目标**：
> - 解释 数据工程全景 ⭐⭐⭐ 的核心问题、机制与适用边界。
> - 实现或评估 语料采集与清洗 ⭐⭐⭐⭐ 的最小闭环。
> - 使用可复现证据诊断 数据增强与合成数据 ⭐⭐⭐⭐ 的工程取舍与失败模式。
>
> **建议路径**：数据工程全景 ⭐⭐⭐ → 语料采集与清洗 ⭐⭐⭐⭐ → 数据增强与合成数据 ⭐⭐⭐⭐ → SFT 数据构建 ⭐⭐⭐⭐ → 数据质量评估 ⭐⭐⭐ → 偏好与 Agent 轨迹数据 → 生产边界与面试表达。
>
> **配套代码**：`code/ch29_data_engineering/`。

本章先回答“数据工程全景 ⭐⭐⭐”为什么成立，再沿着机制、实现、评估和边界逐步展开。阅读时先建立因果链，再运行或推演示例，最后用章末自测检查能否脱离原文复述。
## 29.1 数据工程全景 ⭐⭐⭐
...
> 📚 **交叉引用**：Scaling Law 的详细讨论见 [[15_Transformer架构与实现]] 12.4 节。

### 29.1.3 数据工程 Pipeline 全景图
...
> 📚 **交叉引用**：偏好对齐的详细方法（RLHF/DPO/GRPO）见 [[30_SFT_LoRA与QLoRA]] 16.5 节。
...
### 29.6.3 Agent Trajectory 数据：多轮 + Tool Use ⭐⭐⭐⭐⭐
...
> 📚 **交叉引用**：Agent 框架与执行机制见 [[22_Agent基础与工具调用]]，多轮对话数据格式见 [[30_SFT_LoRA与QLoRA]]。

### 29.6.4 合成数据风险：模型坍塌深度解析 ⭐⭐⭐⭐
...
> 📚 **交叉引用**：模型安全与对抗见 [[38_大模型与Agent安全]]，模型评估污染问题见 [[36_大模型评估基础]]。
...
> 📚 **交叉引用**：多模态架构与训练详见 [[47_多模态表征与多模态大模型]]。

### 29.6.6 Constitutional AI 数据
...
> 📚 **交叉引用**：对齐方法与 RLHF 详见 [[30_SFT_LoRA与QLoRA]] 16.5 节，安全对齐与红队测试详见 [[38_大模型与Agent安全]]。

## 29.7 数据 Pipeline 工具链 ⭐⭐⭐
...
### 29.7.2 Datatrove（HuggingFace 大数据处理）
...
### 29.7.3 TextPack 与 Data-Juicer 对比
...
- 数据工程全景 ⭐⭐⭐：能够说清问题、机制、证据与边界。
- 语料采集与清洗 ⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- 数据增强与合成数据 ⭐⭐⭐⭐：能够说清问题、机制、证据与边界。

## ✅ 自测与练习

1. 不看正文，解释“数据工程全景 ⭐⭐⭐”解决什么问题，并给出一个不适用场景。
2. 为“语料采集与清洗 ⭐⭐⭐⭐”设计一个最小可复现实验，明确输入、指标和通过条件。
3. 比较“数据增强与合成数据 ⭐⭐⭐⭐”的至少两种方案，说明质量、成本、延迟或风险取舍。

## 🧪 配套代码与验收

- `code/ch29_data_engineering/`
...
| 主题 | 回答主线 |
|---|---|
| 数据工程全景 ⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 语料采集与清洗 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 数据增强与合成数据 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| SFT 数据构建 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 数据质量评估 ⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
