...
> [!abstract] 本章导航
> **定位**：全书起点，建立后续 Web、数据与大模型代码共同依赖的 Python 基础。
>
> **先修**：无硬性先修。
>
> **学习目标**：
...
>
> **建议路径**：Python 语言概览与安装环境 → 基础语法速通 → 核心数据结构 → … → 异常处理与编程规范。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch01_python_basics/`。

> [!info] 阅读提示
> 本章覆盖后续章节共同依赖的 Python 语法与数据结构。基础语法本身通常不是深挖重点，但引用关系、参数传递和数据结构边界经常用于检验对象语义是否掌握牢固。
...
1. 你能否解释 Python 执行模型、核心语法和常用数据结构？
2. 你能否编写包含函数、模块和异常处理的可运行程序？
3. 你能否诊断参数传递、作用域和数据结构相关的常见陷阱？

## 🧪 配套代码与验收

配套目录：`code/ch01_python_basics/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch01 --parallel 1 --timeout 60
```
