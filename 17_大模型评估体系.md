...
## 17.10 2026 评估新栈衔接：从指标到平台 ⭐⭐⭐⭐

> **2026 年技术拐点**：LLM 评估正从"自研脚本 + 临时指标"演进为"专业可观测性平台 + 标准化基准 + Eval-as-Code 范式"。Langfuse v3、Phoenix、DeepEval、lm-evaluation-harness 共同构成新一代评估技术栈。

| 演进维度 | 2024 之前 | 2026 新栈 | 驱动力 |
|---------|----------|----------|-------|
| **可观测性** | 自研日志 + 数据库 | OpenTelemetry-native + ClickHouse | 千万级 trace 的存储成本 |
| **基准测试** | 各自实现评估脚本 | lm-eval-harness 标准化 | 论文可复现 + 模型对比公平性 |
...
| **安全护栏** | 黑盒 API 调用 | 开源 gpt-oss-safeguard | 隐私合规 + 低延迟部署 |

---

## 17.11 2026 年评估与可观测性新栈 ⭐⭐⭐⭐⭐

### 17.11.1 全景图：评估新栈分层架构

```mermaid
graph TD
    subgraph "2026 LLM 评估新栈分层架构"
        OBS["可观测性层<br/>Langfuse v3 · Phoenix · LangSmith"]
...
### 17.11.2 Langfuse v3 - 评估与可观测性统一平台

**Langfuse v3**（2025-2026 主流版本）将 LLM 可观测性、Prompt 管理、评估、LLM-as-Judge 整合到同一平台，从 Langfuse v2 的 PostgreSQL 单一后端演进为 ClickHouse 事件存储 + OTel 协议。

**核心特性对比表**：

| 特性 | 说明 | 价值 |
|------|------|------|
| **ClickHouse 后端** | 用 ClickHouse 替代 PostgreSQL 存储 trace | 单表可存数十亿 trace，列式压缩比 ~10x，写入吞吐从 ~10K/s 提升到 ~1M/s |
| **OpenTelemetry-native** | 完全遵循 OTel 协议，与 Grafana/Tempo/Jaeger/Datadog 互通 | 可复用现有 observability 基础设施 |
| **Prompt 管理** | 集中式 prompt 注册表，支持版本控制、AB 测试、环境隔离 | prompt-as-code，与评估 / CI 集成 |
| **LLM-as-Judge** | 内置 60+ judge 模板（relevance / helpfulness / safety / hallucination） | 开箱即用的多维度评估 |
| **Dataset & Experiment** | 数据集版本化 + 实验对比（类似 DVC / MLflow） | 评估结果可复现，可对比回归 |
| **多模态支持** | 文本 / 图像 / 音频 trace 统一存储 | 覆盖多模态场景 |

**Langfuse v3 架构图**：

```mermaid
graph LR
    subgraph "Langfuse v3 架构"
        APP["应用 SDK<br/>Python / JS / TS"] --> SDK["OpenTelemetry SDK"]
        SDK --> COLLECTOR["OTel Collector<br/>(langfuse OTel receiver)"]
        COLLECTOR --> CH["ClickHouse<br/>Trace 存储"]
        COLLECTOR --> PG["PostgreSQL<br/>元数据 / 用户 / Prompt"]
        CH --> UI["Web UI"]
        PG --> UI
        CH --> API["REST / GraphQL API"]
    end
...
**Langfuse v3 vs v2 关键差异**：

| 维度 | Langfuse v2 | Langfuse v3 |
|------|------------|------------|
| 存储后端 | PostgreSQL | ClickHouse（事件表） + PostgreSQL（元数据） |
| 协议 | 自研 SDK | OpenTelemetry-native |
| 扩展性 | 单机 PG | 分布式 ClickHouse 集群 |
| 写入吞吐 | ~10K events/s | ~1M events/s |
| 数据保留成本 | 高 | 降低 5-10x（列式压缩） |
| 集成生态 | 仅 Langfuse | OTel 生态（Grafana / Tempo / Datadog / Honeycomb） |
| Prompt 管理 | 基础 | 完整版本控制 + A/B + 环境隔离 |
| 评估能力 | 简单 score 上报 | 内置 LLM-as-Judge + Experiment Tracking |

**代码示例 1：Langfuse v3 评估与 Prompt 管理一体化**：
...
result = evaluate(
    data=[{"input": "原文本", "output": summarize("原文本")}],
    evaluators=[{
        "name": "relevance",
...
### 17.11.3 Arize Phoenix - 开源 LLM 可观测性标杆

**Phoenix**（Arize AI 开源）是 LLM 可观测性领域的标杆项目，以 **OpenInference** 协议为核心，强调**Auto-Instrumentation**（零代码一行启动）。

**核心组件表**：

| 组件 | 功能 | 关键能力 |
|------|------|---------|
| **OpenInference SDK** | 自动 instrument OpenAI / Anthropic / LangChain / LlamaIndex / Haystack | 零代码一行启动 trace |
| **Phoenix Server** | 自托管 UI（FastAPI + React + SQLite / Postgres） | 实时 trace、span、token 统计 |
| **@arizeai/phoenix-mcp** | Phoenix 的 MCP Server 集成 | Claude / Cursor / Cline 直连 Phoenix 数据 |
| **Phoenix PXI** (Phoenix eXtensions Integrations) | 与 Arize AX / Phoenix Cloud 桥接 | 企业级 trace 存储与告警 |
| **Evaluations API** | 内置 30+ 评估器（hallucination / relevance / QAS / summarization） | 离线批量评估 + 在线 span 评估 |

**Phoenix OpenInference 协议栈**：

```mermaid
graph LR
    subgraph "Phoenix OpenInference 协议栈"
        APP["应用代码"] -->|"@instrument"| OTEL["OpenTelemetry<br/>Exporter"]
        OTEL --> COLLECTOR["Phoenix Collector"]
        COLLECTOR -->|"OTLP"| PS["Phoenix Server<br/>(SQLite/Postgres)"]
        COLLECTOR -->|"gRPC"| CLOUD["Arize Cloud<br/>(生产)"]
        PS --> UI["Phoenix UI"]
        CLOUD --> AX["Arize AX<br/>监控告警"]
    end
...
**代码示例 2：Phoenix Auto-Instrumentation + MCP + 评估**：
...
### 17.11.4 DeepEval - 评估即代码（Eval-as-Code）

**DeepEval** 在 2025-2026 完成了重大架构升级：引入 **DAG-based evaluation**（基于有向无环图的多步评估）和 **G-Eval** 框架。

**DeepEval 核心能力表**：

| 能力 | 说明 | 优势 |
|------|------|------|
| **DAG Evaluator** | 把复杂评估拆成 DAG 节点，逐节点执行 | 可组合、可缓存、并行执行 |
| **G-Eval** | 任意自然语言评估标准的 LLM 实现 | 无需写代码定义 metric，领域专家也能上手 |
| **Pytest 风格** | `assert_test` + `deepeval test run` | 与 CI/CD 天然集成 |
| **Dashboard** | Confident AI 提供的云端评估仪表板 | 历史趋势、回归检测、团队协作 |
| **Regression Testing** | 自动比对 baseline vs 当前 | 检测能力退化（catastrophic forgetting） |
| **30+ 内置指标** | Faithfulness / Relevance / Hallucination / Bias / Toxicity 等 | 开箱即用 |

**DeepEval DAG 评估结构**：

```mermaid
graph TD
    subgraph "DeepEval 评估 DAG 示例"
        TC["Test Case<br/>input/output/context"] --> N1["Node 1<br/>Faithfulness"]
        TC --> N2["Node 2<br/>Relevance"]
        N1 --> N3["Node 3<br/>Coherence"]
        N2 --> N3
        N3 --> OUT["综合评分<br/>Composite Score"]
    end
...
from deepeval.metrics import (
    DAGMetric, GEval, FaithfulnessMetric, AnswerRelevancyMetric
)
from deepeval.metrics.dag import DeepAcyclicGraph
from deepeval.metrics.indicators import (
    TaskCompletionIndicator, HallucinationIndicator
)
...
dag.add_node("No Hallucination", no_hallucination, dependencies=["Task Completion"])
...
### 17.11.5 lm-evaluation-harness - 标准化基准测试平台

**lm-evaluation-harness**（EleutherAI 维护）是行业事实标准的开源 LLM 评估框架，几乎所有开源模型的论文都使用它报告结果。

**2026 版本关键能力表**：

| 能力 | 说明 | 性能影响 |
|------|------|---------|
| **60+ 内置 benchmarks** | MMLU / HumanEval / GSM8K / BBH / HellaSwag / TruthfulQA / GPQA / IFEval 等 | 一行命令运行全套 |
| **vLLM 后端** | 直接对接 vLLM 高吞吐推理 | 评估速度提升 5-20x |
| **SGLang 后端** | 适配 SGLang 推理引擎 | 支持 RadixAttention 加速，再提速 2-3x |
| **HF Transformers** | 兼容 HuggingFace 生态 | 模型无缝加载 |
| **多 GPU 分布式** | torchrun / accelerate 分布式评估 | 线性扩展 |
...
|------|---------|---------------------|
| 启动成本 | 高（自己写 prompt、解析、评分） | 极低（`lm_eval --tasks mmlu`） |
| 标准化 | 各家不统一 | 行业标准，结果可比 |
| 推理后端 | 单一 | HF / vLLM / SGLang / NeMo |
| 性能优化 | 难 | 内置 batch 优化、fewshot caching |
| 结果可复现 | 难 | 锁定任务版本 + config |
| 社区维护 | N/A | 60+ contributors 持续更新 |
| 论文采用 | N/A | 95%+ 开源模型论文 |

**lm-evaluation-harness 典型用法**：

```bash
# 1. 安装（推荐 extras 包含 ifeval / math 任务）
pip install lm-eval[ifeval,math]

# 2. 使用 HF Transformers 后端评估
lm_eval --model hf \
...
# 4. 使用 SGLang 后端（进一步加速 + 复杂 prompt 优化）
lm_eval --model sglang \
    --model_args pretrained=meta-llama/Llama-3.3-70B-Instruct \
    --tasks ifeval,bbh \
    --batch_size 64

# 5. 分布式多卡（8 卡 H100 跑 70B 模型）
accelerate launch --num_processes 8 -m lm_eval \
    --model hf --model_args pretrained=meta-llama/Llama-3.3-70B-Instruct \
    --tasks mmlu --batch_size 4

# 6. 仅跑特定子任务
lm_eval --model vllm --model_args pretrained=... \
    --tasks mmlu_pro --num_fewshot 0 \
    --limit 100  # 仅前 100 题调试
```

### 17.11.6 Trajectory Evals - Agent 全过程评估

**Trajectory evaluation**（轨迹评估）关注 Agent 整个执行路径的"过程质量"，而非仅看最终结果。这是 2026 Agent 评估的核心范式，由 **HuggingFace Agents Course Unit 2/4** 系统化提出。

**轨迹评估核心维度表**：

| 评估维度 | 计算方式 | 工具 |
|---------|---------|------|
| **Step-level correctness** | 每一步 action 是否合理 | Levenshtein / embedding similarity |
| **Goal completion** | 是否达到最终目标 | 与 ground truth final state 比对 |
| **Trajectory match** | 完整轨迹与参考轨迹的匹配度 | SequenceMatcher / edit distance |
| **Token efficiency** | 完成任务的 token 消耗 | `tokens / success_rate` |
| **Error recovery** | 出错后能否自我纠正 | 失败步后是否回退 / 反思 |
| **Tool selection accuracy** | 工具选择是否最优 | Top-1 / Top-k accuracy |
...
|------|------------------|------------------|
| 关注点 | 最终输出 | 整个决策路径 |
| 调试友好度 | 低（黑盒） | 高（可定位错误步骤） |
| 优化指导 | 弱 | 强（可针对性优化） |
| 计算成本 | 低 | 中等（需记录 trace） |
| 适用场景 | 简单 LLM 任务 | 多步 Agent 任务 |
...
### 17.11.7 EvalPlus 与 EvalPerf - 代码与性能专项评估

| 工具 | 评估目标 | 核心特性 | 2026 新进展 |
|------|---------|---------|------------|
| **EvalPlus** | 代码生成质量 | HumanEval / MBPP 增强版（×80 测试用例） | 加入 LiveCodeBench 对接、Contamination Check |
| **EvalPerf** | 代码性能评估 | 评估生成代码的执行效率（runtime / memory） | 新增能耗评估、跨硬件 benchmark |

**EvalPlus 核心数据**：

- **HumanEval+**：将原 HumanEval 164 题扩展到 ~10,000 测试用例（×80 覆盖）
- **MBPP+**：MBPP 378 题扩展，覆盖更全面的边界条件
- **LiveCodeBench** 集成：自动检测训练数据污染

**EvalPerf 综合性能得分公式**：

$$\text{Performance Score} = w_1 \cdot \text{Speedup} + w_2 \cdot \text{Memory Ratio} + w_3 \cdot \text{Energy Efficiency}$$

其中 $w_1 + w_2 + w_3 = 1$，权重可由业务场景调整（嵌入式偏 $w_2$，数据中心偏 $w_1$）。

### 17.11.8 gpt-oss-safeguard - 开源安全护栏模型

**gpt-oss-safeguard**（OpenAI 2025 末开源）是面向安全护栏场景的开源分类模型系列，专为**输入安全检查、输出合规审查、越狱检测**设计。
...
| **训练目标** | 多标签安全分类（toxicity / pii / jailbreak / csam 等） |
| **部署方式** | 自托管（vLLM / SGLang / Triton） |
| **延迟** | <50ms（gpt-oss-safeguard-20b on H100） |
| **可定制** | 类别定义 + few-shot prompt 即可适配新场景 |
| **许可** | Apache 2.0（自托管） + OpenAI 商用 API |

**gpt-oss-safeguard 评估能力表**：

| 评估任务 | 准确率 | 误报率 | 延迟（P99） |
|---------|--------|--------|------------|
| 越狱检测 | 96.2% | 0.8% | 35ms |
| PII 识别 | 98.7% | 0.3% | 28ms |
| 有害内容分类 | 94.5% | 1.2% | 40ms |
| CSAM 检测 | 99.1% | 0.1% | 50ms |
| CSAM（强制 hash 前置） | - | - | < 5ms |

### 17.11.9 2026 端到端评估流水线

```mermaid
graph LR
    subgraph "2026 端到端评估流水线"
        CODE["应用代码"] --> PHOENIX["Phoenix<br/>OpenInference Trace"]
        PHOENIX --> LANGFUSE["Langfuse v3<br/>评估 + Prompt"]
...
| 工具 | 核心定位 | 适用阶段 | 替代方案 |
|------|---------|---------|---------|
| **Langfuse v3** | 可观测性 + Prompt 管理 | 生产监控 + CI | LangSmith / Helicone |
...
| 概念 | 关键点 |
|------|--------|
| 自动评估指标 | BLEU/ROUGE 表面 n-gram 匹配，BERTScore 上下文嵌入余弦相似度，Perplexity 反映流畅度 |
| LLM-as-Judge 范式 | 用强模型评审弱模型输出，含 Pointwise 打分与 Pairwise 成对比较，MT-Bench/Chatbot Arena 是代表基准 |
| Judge 偏见与缓解 | 位置/长度/自我增强/格式/风格五大偏见，采用位置交换 + 多次评估 + Pairwise 取平均 |
| 主流基准测试 | MMLU（57 学科知识）、HumanEval/MBPP（pass@k 代码）、GSM8K/MATH（推理）、TruthfulQA（真实性） |
| RAG 评估三件套 | Ragas（忠实度/相关性/上下文指标）、TruLens（RAG Triad）、DeepEval（类 pytest + DAG + G-Eval） |
| Agent 评估指标 | 任务成功率 SR、工具调用准确率 TA、Efficiency Score = SR × S_min/S_actual，关注多步错误累积 |
| 安全与红队测试 | 越狱检测（规则+LLM 双层）、有害内容分类、偏差公平性（Demographic Parity），防御深度原则 |
| 评估新栈 2026 | Langfuse v3（OTel + ClickHouse）、Phoenix（OpenInference）、DeepEval（DAG Eval-as-Code）、lm-eval-harness 标准化 |
| Eval-as-Code 范式 | DeepEval assert_test + deepeval test run 接入 CI/CD，自动回归检测与灾难性遗忘 |
| 评估-优化闭环 | Eval-Driven Development → 短板识别 → 定向 SFT/DPO → 回归评估 → A/B 赢率分析 → 生产监控 |
| 配套代码 | `ch17_evaluation/llm/06_ragas_evaluation.py` W3 真实化, 用本地 bge + DeepSeek LLM judge | 需 `export DEEPSEEK_API_KEY=...` + 真实 API |

## 17.x 配套代码真实化 (Wave 5 完成)

`code/ch17_evaluation/llm/06_ragas_evaluation.py` 已 W3 真实化:

- 用 `sentence_transformers` 加载本地 `bge-small-zh-v1.5` embedding (无需 OpenAI API)
- 用 `langchain_openai.ChatOpenAI + DeepSeek base_url` 跑 LLM judge
- 真实跑 RAGAS 评估: faithfulness / answer_relevancy / context_precision 等
...
下载 bge 模型:
```bash
make download-models-default  # bge-small-zh 已含
```
