...
> [!abstract] 本章导航
> **定位**：构建外部知识增强闭环，是企业知识问答和 Agent 检索能力的核心。
>
> **先修**：[[12_Transformer与大模型原理]]、[[13_Prompt_Engineering]]。
>
> **学习目标**：
...
> - 根据坏例区分检索、上下文和生成问题。
>
> **建议路径**：RAG 概述与价值 → RAG 完整架构 → 文档处理与分块策略 → … → RAG 评估与优化。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch14_rag/`。

> [!info] 阅读提示
> RAG（Retrieval-Augmented Generation）是大模型落地企业场景的常用架构。它将大模型的生成能力与外部知识检索相结合，用于缓解模型知识过时、幻觉和无法访问私有数据等问题。本章从架构原理到高级优化，从向量索引到评估体系，串联 RAG 技术栈的关键环节。
>
> **版本与范围**：新增 RAG 与 Agent 融合（RAG-as-a-Tool）、多模态 RAG、MCP 集成、端云协同 RAG 等截至审校日的技术方向。
...
1. 你能否解释 RAG 从摄取、索引、检索到生成的完整链路？
2. 你能否实现并评估一个可追踪来源的最小 RAG 系统？
3. 你能否根据坏例区分检索、上下文和生成问题？
...
向量检索偏向语义匹配（如“大语言模型”匹配“LLM”），BM25 偏向词项匹配（如型号、ID）。二者可以互补，但融合是否提高召回率应在目标查询集上评测。
...
**回答要点**：在 Agent 架构中，RAG 通常作为可调用的检索能力参与任务，而不是独立承担完整编排；是否采用这一结构取决于任务状态和工具边界。
