...
> [!abstract] 本章导航
> **定位**：建立 Python 面向对象模型，为框架源码、依赖注入和设计模式打基础。
>
...
>
> **建议路径**：类与对象基础 → 面向对象三大特性 → 多继承与 C3 线性化算法 → … → 元类 Metaclass。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch03_oop/`。
...
1. 你能否解释类、对象、继承、描述符和元类的协作关系？
2. 你能否实现职责清晰且可测试的类层次？
3. 你能否诊断 MRO、属性查找和对象构造相关问题？

## 🧪 配套代码与验收

配套目录：`code/ch03_oop/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch03 --parallel 1 --timeout 60
```
