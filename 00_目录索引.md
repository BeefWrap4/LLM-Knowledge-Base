...
> 根据你的硬件选择可跑的章节, 避免在无 GPU 时尝试 GPU 章节浪费时间.

| 硬件 | 可跑章节 | 必装命令 |
|:---|:---|:---|
| **任意笔记本** | Ch1-11 + Ch13/14/15/17/18/20/22/29 (用 API Key) | `pip install -r requirements-llm.txt` |
...
| **NVIDIA GPU 24GB+** (e.g. RTX 5090 D) | + Ch25 vLLM 7B + Ch27 R1-Distill + Ch16/26 真训 | `make download-models-llm` |
| **NVIDIA GPU 80GB+** (A100/H100) | + Ch26 Cosmos-7B / Pi0 VLA | `make download-models-gpu` |

> 💡 推荐: 注册 DeepSeek (https://platform.deepseek.com) 拿 ¥10 体验金, 国内访问快, OpenAI 协议, 默认推荐.
...
# 2. 配置 API Key (推荐 DeepSeek, 注册送 ¥10)
export DEEPSEEK_API_KEY=sk-xxx
# 国内访问快, OpenAI 协议, 默认推荐

# 3. 下载模型权重
make download-models-default  # 1.7GB (embedding + reranker + Qwen2.5-0.5B)
...
- ⚡ **面试高频 Top 5**：Self-Attention / QKV | LoRA / QLoRA | RAG 完整架构 | KV Cache | ZeRO 优化 | PagedAttention | GRPO

## 🎯 Wave 6 真实化真题精讲

> 配套代码 W1-W6 完成 35 commits 真实化, 450 个 .py 全部可真实运行, 面试时可现场 demo.

**Q1: 配套代码用 mock 还是真实 API? 现在能跑真实模型吗?**
A: W3-W4 完成真实化, 默认走 DeepSeek (OpenAI 协议兼容). 设置 `export DEEPSEEK_API_KEY=sk-xxx` 后所有 `ch13-18/llm/*.py` 走真实 API. 无 Key 时 `LLM_MOCK=1` 跑 mock 不报错. Ch19/25/26/27 章节用本地 bge/Qwen2.5-7B/Cosmos 等真实模型, 需 24GB+ NVIDIA GPU.
...
**Q5: 测试如何组织? 有没有 CI?**
A: `code/tests/` 含 `test_pilots.py` 冒烟测试, 跑 `pytest tests/ -m "not gpu"` 验证 core+llm. CI 在 `.github/workflows/integration-test.yml`, 支持 REDIS_HOST/PG_HOST 环境变量, 验证 ch19 DDP 多卡 + ch25 vLLM 集成.
