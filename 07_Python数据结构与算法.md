...
tags:
  ...
  - 面试手写
---
# 第 7 章 Python 数据结构与算法 ⭐⭐⭐⭐⭐

> [!abstract] 本章导航
> **定位**：建立复杂度与数据结构直觉，为检索、调度和算法面试提供工具。
>
> **先修**：[[01_Python编程基础]]。
>
> **学习目标**：
...
> - 解释时间、空间、可维护性之间的工程取舍。
>
> **建议路径**：基础数据结构 → 树与图 → 排序算法 → … → LeetCode 面试刷题指南。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch07_data_structures/`。

数据结构与算法是常见的技术面试环节，重点考察问题建模、复杂度分析和代码正确性。本章系统讲解面试高频数据结构和算法，每个知识点附带完整 Python 实现和复杂度分析，最后提供必刷 50 题清单。
...
1. 你能否根据访问模式选择合适的数据结构并分析复杂度？
2. 你能否实现常见排序、图、树和动态规划算法？
3. 你能否解释时间、空间、可维护性之间的工程取舍？

## 🧪 配套代码与验收

配套目录：`code/ch07_data_structures/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch07 --parallel 1 --timeout 60
```
