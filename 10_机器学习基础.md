...
> [!abstract] 本章导航
> **定位**：建立传统机器学习问题求解闭环，为深度学习和模型评估提供基线。
>
> **先修**：[[07_Python数据结构与算法]]、[[08_Python数据科学核心库]]。
>
> **学习目标**：
...
> - 诊断数据泄漏、类别不平衡和过拟合问题。
>
> **建议路径**：机器学习概述 → 经典监督学习算法 → 集成学习 → … → Scikit-learn 实战：完整建模流程。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch10_ml_basics/`。

机器学习是大模型技术的理论根基。经典算法原理、模型评估指标和偏差—方差权衡，仍是理解大模型训练与评估的重要基础。本章系统梳理面试高频考点，兼顾原理深度与实践代码。
...
1. 你能否解释监督学习、无监督学习和评估的基本假设？
2. 你能否实现包含预处理、训练和交叉验证的基线流程？
3. 你能否诊断数据泄漏、类别不平衡和过拟合问题？

## 🧪 配套代码与验收

配套目录：`code/ch10_ml_basics/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch10 --parallel 1 --timeout 60
```
