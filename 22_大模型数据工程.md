...
## 22.5 RLHF/Agent Trajectory 数据工程 ⭐⭐⭐⭐⭐

> 🆕 **2026年焦点**：随着 o1/o3、DeepSeek-R1、Claude 3.5 Sonnet 等推理模型的崛起，以及 Agent 应用的工业化落地，**RLHF/RLAIF 数据**、**Agent Trajectory 数据**、**长 CoT 数据**已成为 2026 年大模型数据工程的核心战场。本节深入解析这一系列前沿数据工程实践，涵盖偏好数据设计、Agent 轨迹采集、合成数据风险、多模态数据策划、推理模型数据以及 Constitutional AI 全链路。

### 22.5.1 RLHF/RLAIF 数据构建

RLHF（Reinforcement Learning from Human Feedback）和 RLAIF（RL from AI Feedback）是大模型对齐阶段最重要的数据工程任务。从 InstructGPT 到 GPT-4，从 Llama-2-Chat 到 Claude 3.5，所有顶尖模型的对齐成功都建立在高质量的偏好数据之上。

**RLHF 数据构建的三层结构**：

| 阶段 | 数据类型 | 规模 | 来源 |
|-----|---------|------|------|
| **Reward Model 训练** | 偏好对 chosen/rejected | 10K-100K 对 | 人工标注 + 模型生成 |
| **PPO/DPO 训练** | Prompt + Multi-response | 50K-500K | Prompt 池 + 策略采样 |
| **在线 RL** | Prompt → 实时反馈 | 持续收集 | 用户交互日志 |

**RLAIF 的兴起（2024-2026 主流方案）**：

RLAIF 用 AI 反馈替代部分人类反馈，大幅降低成本，已成为 2026 年的标配方案：

- **Anthropic Constitutional AI**：基于 Constitution 规则集生成 AI 偏好（见 22.5.7）
- **UltraFeedback**：用 GPT-4 对回复进行多维度评分（4 个维度 × 5 级评分），开源 64K 偏好对
...
研究反复发现，奖励模型倾向于给长回复更高分（即使长回复并不更好），这是 RLHF 中最常见的 reward hacking 形式。Anthropic 在 2023 年的 paper 中指出，未做长度校准的 RM 在偏好判断中 ~60% 的分数来自长度信号。

**缓解策略**：
- **Length-Normalized Reward**：$R_{\text{norm}} = R / \log(L)$
- **长度匹配采样**：在构建偏好对时强制 |L(chosen) - L(rejected)| < 阈值
- **DPO 中的 length penalty**：在损失中显式惩罚长度
- **R-DPO / IPO**：改进的对齐目标，减少长度依赖
...
### 22.5.3 Agent Trajectory 数据：多轮 + Tool Use ⭐⭐⭐⭐⭐
...
graph TB
    ...
    subgraph "数据采集来源"
        ...
        S2["🤖 强模型 Trace 蒸馏<br/>GPT-4 / Claude 3.5 trace"]
...
| 数据集 | 任务类型 | 规模 | 特点 |
|-------|---------|------|------|
| **ToolBench** | 通用工具调用 | 16K 工具 + 100K 任务 | 工具种类丰富 |
| **AgentInstruct** | 多场景 Agent | 1866 高质量任务 | Microsoft 高质量数据 |
| **WebArena / VisualWebArena** | 网页 Agent | 真实网站任务 | 视觉 + 操作 |
| **SWE-bench / SWE-Gym** | 代码修复 Agent | 2294 个 GitHub Issue | 真实软件工程 |
| **xLAM** | Function calling | 60K 多步样本 | Salesforce 开源 |
| **Glaive Function Calling V2** | API 调用 | 113K 样本 | 主流 SFT 起点 |
| **OpenHands Trajectories** | 通用 Coding Agent | 10K+ | 2025 真实任务轨迹 |
| **APIGen** | 函数调用合成 | 60K 高质量 | 自动验证流程 |
...
$$\lim_{n \to \infty} \text{Var}(P_n) \to 0$$

实证显示，即使每代有 90% 合成数据 + 10% 真实数据混合训练，模型也会在 5-10 代后出现明显退化。
...
| 策略 | 原理 | 实施难度 |
|-----|------|---------|
| **真实数据锚定** | 训练数据中保留 ≥20% 真实数据，并按真实数据 epoch 多次回访 | 低 |
...
多模态大模型（如 GPT-4V、Claude 3.5 Sonnet、Gemini 2.0、Qwen2-VL）的崛起带来了全新的数据工程挑战。**多模态数据的策划复杂度比纯文本高一个数量级**。
...
6. **高分辨率筛选**：低分辨率图片（< 256x256）对视觉理解贡献有限
7. **细粒度 Caption 升级**：用 GPT-4V/InternVL 重新生成细致 caption，替换原始 alt-text
...
### 22.5.6 推理模型数据：R1-Distill 与 s1 长 CoT 策划 ⭐⭐⭐⭐⭐
...
| 维度 | 普通 SFT 数据 | 推理模型数据 |
|-----|------------|------------|
| **回复长度** | 100-1000 tokens | 1000-32000 tokens |
| **结构** | 直接回答 | Think + Answer 双段 |
| **Think 内容** | 无 | 反思、回溯、自我修正、Aha-moment |
| **正确性** | 关注答案正确 | 关注推理路径正确 |
| **数据规模** | 10K-1M | 1K-100K（少而精）|
...
DeepSeek-R1 开源了 **800K 条** 高质量推理数据，覆盖数学、代码、科学推理等多个领域，已成为开源推理模型训练的标准起点。

**s1 数据集的关键设计（Stanford 2025）**：

s1 的核心发现：**仅用 1000 条** 精心挑选的长 CoT 数据微调 Qwen2.5-32B，即可达到接近 o1-preview 的推理能力。这是继 LIMA 之后又一个"少即是多"的里程碑。

**s1 数据筛选三大标准**：

1. **Difficulty（难度）**：题目必须有足够难度，GPT-4o / Claude 3.5 Sonnet 也需要思考
2. **Diversity（多样性）**：覆盖 50+ 领域（数学奥赛、博士级物理、量子计算、金融建模等）
3. **Quality（质量）**：推理轨迹由 Gemini Thinking 生成并人工筛选，确保推理路径正确
...
### 22.5.7 Constitutional AI 数据

Constitutional AI（CAI，Anthropic 2022 提出）是 RLAIF 的开山之作，核心思想：**用一套显式的"宪法原则"（Constitution）指导 AI 进行自我批评和修正，生成训练数据**。
...
**Constitution 示例（Anthropic Claude 公开原则节选）**：
...
... （Anthropic 实际有 ~75 条原则，部分参考世界人权宣言、Apple ToS、DeepMind Sparrow 规则等）
```

**CAI 数据的优势**：

1. **可扩展性**：不需要大量人工标注，AI 自我生成，成本降低 10-100 倍
2. **可控性**：通过修改 Constitution 即可调整对齐目标
3. **透明性**：每条偏好都有显式的原则支撑，便于审计
4. **一致性**：避免人工标注者间的不一致（IAA 问题）
...
- **Collective Constitutional AI**：Anthropic 通过 Polis 等民主工具收集大众意见形成 Constitution
- **Domain-Specific Constitution**：医疗、法律、教育等垂直领域定制原则
- **Adaptive Constitution**：根据用户/场景动态调整 Constitution
- **Constitutional Classifier**：将原则蒸馏为分类器，提升推理效率
- **Multi-Constitution Ensemble**：多套 Constitution 投票，避免单一价值观偏差
...
| 三层去重 | 精确去重（Hash）→ MinHash LSH（Jaccard 近邻）→ SimHash（海明距离），先快后慢，去重可带来 2-5% 性能提升 |
| 质量过滤 | 启发式规则 + 困惑度评分 + 分类器（FineWeb-Edu），过滤最低 10% 数据可提升 5-15% 性能 |
| PII 脱敏 | 邮箱/电话/IP/身份证用正则匹配，人名用 NER，宁可多脱敏（假阳性）不可漏检（假阴性） |
...
| LIMA 原则 | 1000 条精选高质量数据 > 52K 条普通数据，质量 >> 数量，但数学/代码等能力注入场景有局限 |
| 质量评估 | 六维框架（准确性/完整性/一致性/多样性/时效性/无毒性），自动评分（LLM-as-Judge）+ 分层人工抽检 |
| 模型坍塌 | 反复用合成数据训练导致尾部信息丢失，需保留 ≥20% 真实数据 + 多源混合 + 水印追踪 |
| 工具链 | Datatrove（HF）/ Data-Juicer（阿里）/ NeMo Curator（NVIDIA），分布式选 Spark/Ray，版本管理用 DVC |
| 配套代码 | `ch22_data/llm/*.py` 全部 W3 真实化, 真实 Data-Juicer + DeepSeek 合成 | 需 `export DEEPSEEK_API_KEY=...` + 真实 API |

## 22.x 配套代码真实化 (Wave 6 完成)

本章所有 `.py` 例子已 W3 真实化: 真实 Data-Juicer 数据清洗 + 真实 DeepSeek 合成数据 + 真实 DVC 版本管理.

```bash
# 跑本章例子
export DEEPSEEK_API_KEY=sk-xxx
python ch22_data/llm/01_data_quality_filter.py   # 六维质量过滤
python ch22_data/llm/05_synthetic_data.py          # 合成数据生成
python ch22_data/llm/10_dvc_versioning.py          # DVC 版本管理
```
