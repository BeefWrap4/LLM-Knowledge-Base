...
# 第 7 章 Python 并发编程 ⭐⭐⭐⭐
> [!abstract] 本章导航
> **定位**：第一部分 Python 与后端工程基础中的第 7 章；围绕“Python 并发编程”建立单一、可追踪的知识主线。
>
> **先修**：[[06_Python内存管理与性能诊断|第 6 章 Python 内存管理与性能诊断]]。
>
> **学习目标**：
> - 解释 GIL 全局解释器锁 ⭐⭐⭐⭐⭐ 的核心问题、机制与适用边界。
> - 实现或评估 进程、线程、协程核心区别 ⭐⭐⭐⭐⭐ 的最小闭环。
> - 使用可复现证据诊断 多线程编程 threading ⭐⭐⭐⭐ 的工程取舍与失败模式。
>
> **建议路径**：GIL 全局解释器锁 ⭐⭐⭐⭐⭐ → 进程、线程、协程核心区别 ⭐⭐⭐⭐⭐ → 多线程编程 threading ⭐⭐⭐⭐ → 多进程编程 multiprocessing ⭐⭐⭐⭐ → 异步编程 asyncio ⭐⭐⭐⭐⭐ → 并发编程选型指南 ⭐⭐⭐⭐⭐。
>
> **配套代码**：`code/ch07_concurrency/`。

本章先回答“GIL 全局解释器锁 ⭐⭐⭐⭐⭐”为什么成立，再沿着机制、实现、评估和边界逐步展开。阅读时先建立因果链，再运行或推演示例，最后用章末自测检查能否脱离原文复述。
## 7.1 GIL 全局解释器锁 ⭐⭐⭐⭐⭐
...
### 7.1.4 CPU 密集型 vs IO 密集型任务的并发选型 ⭐⭐⭐⭐⭐
...
### 7.1.5 Python 3.13 nogil 模式详解 （2026年更新）
...
> **回答要点**：free-threaded CPython 通过 biased reference counting、线程本地状态和内部同步降低共享引用计数竞争。它不是简单“删除 GIL”：扩展兼容、单线程开销、共享可变状态和显式锁仍需单独评估。

> [!info] 版本边界
> Python 3.13 首次提供实验性 free-threaded 构建；Python 3.14 已进入“官方支持但仍为可选构建”的阶段。部分不兼容的 C 扩展可能在导入时重新启用 GIL，实际并行收益与单线程开销必须按解释器、扩展、硬件和工作负载实测。参见 [Python free-threading HOWTO](https://docs.python.org/3/howto/free-threading-python.html) 与 [Python 3.14 更新说明](https://docs.python.org/3.14/whatsnew/3.14.html#free-threaded-python-is-officially-supported)。
...
| **并行能力** | 可利用多核 | 默认构建受 GIL 约束；free-threaded 构建可利用多核，但受扩展与工作负载影响 | 单事件循环内提供并发，不等于 CPU 并行 |
...
### 7.5.3 create_task vs gather vs TaskGroup
...
### 7.5.5 实战：用 asyncio 编写高并发 HTTP 客户端
...
- GIL 全局解释器锁 ⭐⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- 进程、线程、协程核心区别 ⭐⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- 多线程编程 threading ⭐⭐⭐⭐：能够说清问题、机制、证据与边界。

## ✅ 自测与练习

1. 不看正文，解释“GIL 全局解释器锁 ⭐⭐⭐⭐⭐”解决什么问题，并给出一个不适用场景。
2. 为“进程、线程、协程核心区别 ⭐⭐⭐⭐⭐”设计一个最小可复现实验，明确输入、指标和通过条件。
3. 比较“多线程编程 threading ⭐⭐⭐⭐”的至少两种方案，说明质量、成本、延迟或风险取舍。

## 🧪 配套代码与验收

- `code/ch07_concurrency/`

```powershell
python code/scripts/run_all_examples.py --chapter ch07 --tier core
```
...
| 主题 | 回答主线 |
|---|---|
| GIL 全局解释器锁 ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 进程、线程、协程核心区别 ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 多线程编程 threading ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 多进程编程 multiprocessing ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 异步编程 asyncio ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |

## 🔗 相关章节

- [[06_Python内存管理与性能诊断|第 6 章 Python 内存管理与性能诊断]]
