...
tags:
  ...
  - MaxText
---
# 第 36 章 JAX 与 TPU 大规模预训练：Pathways、Pallas、XLA ⭐⭐⭐

> [!abstract] 本章导航
> **定位**：补齐 JAX、XLA 与 TPU 训练栈，建立函数变换和显式分片心智模型。
>
> **先修**：[[11_深度学习与PyTorch]]、[[19_分布式训练系统]]、[[33_训练稳定性与诊断]]。
>
> **学习目标**：
...
>
> **建议路径**：JAX 编程模型：函数式、纯函数、变换 → XLA：加速线性代数编译器 → Pallas：自定义加速器 Kernel → … → PyTorch ↔ JAX 转换与迁移。先完成主线，再按需要阅读进阶内容。
>
...
1. 你能否解释 JAX 函数变换、XLA 编译和数组分片？
2. 你能否迁移一个 PyTorch 训练思路并识别语义差异？
3. 你能否根据硬件、生态和规模选择 JAX/TPU 路线？
