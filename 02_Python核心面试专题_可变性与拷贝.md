...
> [!abstract] 本章导航
> **定位**：从语法进入对象语义，解决赋值、共享引用和复制行为容易混淆的问题。
>
> **先修**：[[01_Python编程基础]]。
>
> **学习目标**：
...
> - 根据嵌套结构和性能约束选择复制策略。
>
> **建议路径**：可变类型与不可变类型 → 深拷贝与浅拷贝。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch02_mutability/`。
...
1. 你能否区分对象身份、相等性、可变性与哈希性？
2. 你能否预测赋值、浅拷贝和深拷贝后的引用关系？
3. 你能否根据嵌套结构和性能约束选择复制策略？

## 🧪 配套代码与验收

配套目录：`code/ch02_mutability/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch02 --parallel 1 --timeout 60
```
...
> **章节小结**：可变性与拷贝是 Python 面试的常见专题。核心要点：
