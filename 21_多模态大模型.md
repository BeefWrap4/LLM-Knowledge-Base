...
> [!abstract] 本章导航
> **定位**：从文本模型扩展到视觉、语音和生成模型，建立跨模态表示与对齐直觉。
>
...
> - 评估多模态数据、指标、安全和部署约束。
>
> **建议路径**：多模态概述 → CLIP与对比学习 → 视觉Transformer（ViT） → … → 多模态RAG与Agent。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch21_multimodal/`。

> [!info] 阅读提示
> **一句话总结**：多模态大模型让 AI 能够联合处理文本、图像、音频、视频等多种模态信息，是扩展模型感知与生成能力的重要方向。本章从 CLIP 的对比学习奠基，到 LLaVA 的架构创新，再到扩散模型的生成机制，系统梳理多模态技术的核心原理与截至审校日的进展。
...
1. 你能否解释多模态编码、对齐、融合与生成的主要架构？
2. 你能否比较视觉语言模型、扩散模型和统一模型的能力边界？
3. 你能否评估多模态数据、指标、安全和部署约束？

## 🧪 配套代码与验收

配套目录：`code/ch21_multimodal/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier gpu --chapter ch21 --parallel 1 --timeout 180
```
...
| **DDPM** | 前向加噪+反向去噪 | $\mathcal{L} = \mathbb{E}\lVert\boldsymbol{\epsilon} - \boldsymbol{\epsilon}_\theta(\mathbf{x}_t, t)\rVert^2$ |
