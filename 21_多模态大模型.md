...
- [[26_世界模型与具身AI]]：Sora 2、Genie 3、Cosmos 等世界模型本质上是多模态生成模型的物理一致性扩展，涉及 DiT 时空注意力与可交互性建模。
- [[29_Context_Engineering]]：多模态RAG（ColPali）、长上下文多模态理解（Gemini 3.0 百万token）、多模态Prompt 编排等均属于 Context Engineering 的核心议题。
- [[25_推理引擎与高性能服务]]：Diffusion LLM（LLaDA 2.0、Mercury）的并行解码与 KV Cache 复用、Any-to-Any 多模态模型的统一推理调度，都依赖于 vLLM、SGLang 等高性能推理引擎的支持。
