...
> [!abstract] 本章导航
> **定位**：建立并发模型，为异步 API、任务队列和高吞吐服务提供基础。
>
> **先修**：[[01_Python编程基础]]、[[04_Python高级特性与函数式编程]]。
>
> **学习目标**：
> - 比较线程、进程和协程的调度与隔离边界。
...
>
> **建议路径**：GIL 全局解释器锁 → 进程、线程、协程核心区别 → 多线程编程 threading → … → 并发编程选型指南。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch05_concurrency/`。

并发编程是 Python 后端与高并发岗位的重要基础，中高级岗位常从原理、选型和故障处理继续追问。本章从 GIL 原理出发，系统讲解线程、进程、协程三种并发模型，并通过大量可运行代码和面试真题帮你建立完整的并发编程知识体系。
...
1. 你能否比较线程、进程和协程的调度与隔离边界？
2. 你能否实现能够正确取消、超时和回收资源的并发程序？
3. 你能否根据 CPU、I/O 和故障隔离需求选择并发方案？

## 🧪 配套代码与验收

配套目录：`code/ch05_concurrency/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch05 --parallel 1 --timeout 60
```
