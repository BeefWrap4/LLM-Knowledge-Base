---
chapter: 50
topic: SSM、Mamba 与非 Transformer 架构
topic_id: ssm-mamba
difficulty: 中高
...
tags:
  ...
  - 面试教程
---
# 第 50 章 SSM、Mamba 与非 Transformer 架构 ⭐⭐⭐⭐
> [!abstract] 本章导航
> **定位**：第七部分 多模态与前沿架构中的第 50 章；围绕“SSM、Mamba 与非 Transformer 架构”建立单一、可追踪的知识主线。
>
> **先修**：[[49_世界模型VLA与具身智能|第 49 章 世界模型、VLA 与具身智能]]。
>
> **学习目标**：
> - 解释 SSM 理论基础 ⭐⭐⭐⭐⭐ 的核心问题、机制与适用边界。
> - 实现或评估 Selective SSM 与 Mamba ⭐⭐⭐⭐⭐ 的最小闭环。
> - 使用可复现证据诊断 Mamba-2 与状态空间对偶性 ⭐⭐⭐⭐ 的工程取舍与失败模式。
>
> **建议路径**：SSM 理论基础 ⭐⭐⭐⭐⭐ → Selective SSM 与 Mamba ⭐⭐⭐⭐⭐ → Mamba-2 与状态空间对偶性 ⭐⭐⭐⭐ → 同代架构横评 ⭐⭐⭐⭐ → Hybrid SSM-Transformer 混合架构 ⭐⭐⭐⭐⭐。
>
> **配套代码**：本章暂无独立代码目录。

本章先回答“SSM 理论基础 ⭐⭐⭐⭐⭐”为什么成立，再沿着机制、实现、评估和边界逐步展开。阅读时先建立因果链，再运行或推演示例，最后用章末自测检查能否脱离原文复述。
## 50.1 SSM 理论基础 ⭐⭐⭐⭐⭐

### 50.1.1 为什么需要超越 Transformer
...
## 50.2 Selective SSM 与 Mamba ⭐⭐⭐⭐⭐

### 50.2.1 S4 的局限：内容无关的选择性缺失
...
### 50.3.3 Mamba-3：从 SSD 到更强递推与 MIMO
...
### 50.4.1 RWKV-7：线性递归与广义 delta rule
...
### 50.5.2 MiniMax-01：Lightning Attention 与百万级上下文
...
- SSM 理论基础 ⭐⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- Selective SSM 与 Mamba ⭐⭐⭐⭐⭐：能够说清问题、机制、证据与边界。
- Mamba-2 与状态空间对偶性 ⭐⭐⭐⭐：能够说清问题、机制、证据与边界。

## ✅ 自测与练习

1. 不看正文，解释“SSM 理论基础 ⭐⭐⭐⭐⭐”解决什么问题，并给出一个不适用场景。
2. 为“Selective SSM 与 Mamba ⭐⭐⭐⭐⭐”设计一个最小可复现实验，明确输入、指标和通过条件。
3. 比较“Mamba-2 与状态空间对偶性 ⭐⭐⭐⭐”的至少两种方案，说明质量、成本、延迟或风险取舍。
...
| 主题 | 回答主线 |
|---|---|
| SSM 理论基础 ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| Selective SSM 与 Mamba ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| Mamba-2 与状态空间对偶性 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| 同代架构横评 ⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
| Hybrid SSM-Transformer 混合架构 ⭐⭐⭐⭐⭐ | 问题 → 机制 → 示例 → 指标 → 边界 |
