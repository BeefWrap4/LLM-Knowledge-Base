...
> [!abstract] 本章导航
> **定位**：从单卡训练扩展到多设备系统，建立并行、通信和显存之间的统一模型。
>
> **先修**：[[11_深度学习与PyTorch]]、[[12_Transformer与大模型原理]]、[[16_模型微调与推理优化]]。
>
> **学习目标**：
...
> - 使用利用率和通信证据诊断扩展效率。
>
> **建议路径**：分布式训练概述 → 数据并行 → 模型并行 → … → 2026 年训练技术更新与边界。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch19_distributed/`。

> [!info] 阅读提示
> 当训练状态、激活或目标吞吐超过单卡能力时，就需要数据/模型并行、状态分片
> 或 offload。量化与 CPU offload 可能让单机完成某些大模型推理，但不能替代
> 大规模训练的容量与吞吐设计。本章覆盖常见分布式训练策略及其验收方法。
>
> **版本与范围**：新增 FSDP2（`DTensor` 架构）、DeepSpeed Ulysses 序列并行、FP8 训练实战、通信-计算重叠的前沿技巧。
...
### 19.9.1 Muon 与 Schedule-Free：先看参数域和训练协议
...
### 19.9.3 TLX Block Attention：固定块对角、Blackwell 专用
...
1. 你能否解释数据、张量、流水线并行及 ZeRO 的通信边界？
2. 你能否估算不同并行策略的显存与通信成本？
3. 你能否使用利用率和通信证据诊断扩展效率？

## 🧪 配套代码与验收

配套目录：`code/ch19_distributed/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier gpu --chapter ch19 --parallel 1 --timeout 180
```
...
6. **性能差异**：DDP 通常避免 DP 的 GPU 0 聚合瓶颈并支持通信计算重叠，但实际加速取决于
   模型、batch、互联、bucket 和算子；应在目标 8 卡环境中基准测试，不能给出通用百分比。
...
- **ZeRO Stage 3**：分片**参数 + 梯度 + 优化器状态**。持久化模型状态由
  $16\Phi$ 理想分片为 $16\Phi/N$；模块计算前后的参数物化、释放与预取会造成
  临时峰值和额外 collective，开销必须按实现、互联与重叠效果实测。
...
> **注意**：以上是理想均匀分片的持久化参数相关状态；实际还需加入激活、
> 临时参数物化、通信 bucket、运行时上下文和碎片。
...
- **优点**：降低单个 rank 的层内参数与计算规模；放在高带宽互联域时通常更有效。
- **缺点**：引入频繁的层内 collective；具体是 AllReduce、ReduceScatter、
  AllGather 还是其他通信，以及每层次数，取决于并行实现和网络结构。TP
  可以跨节点，但是否划算必须由拓扑和基准决定。

**Pipeline Parallelism（流水线并行）**：
- **做法**：将模型按层切分，GPU 0 负责 layer 0-7，GPU 1 负责 layer 8-15，数据依次流经各 GPU。
- **优点**：通信量小（仅传递层间激活值）；可以跨节点扩展。
- **缺点**：存在 Bubble；在均衡 GPipe 简化模型中，相对理想计算时间的开销约
  $(p-1)/m$，按总调度时间的空泡占比约 $(p-1)/(m+p-1)$。真实结果还受调度与切分影响。

**关键面试点**：三者可以组合，但映射不是固定规则。通常把通信最频繁的
TP 放在最高带宽域，再按层切分与全局 batch/收敛约束选择 PP 和 DP；最终以
目标集群的容量模型和 collective 基准为准。
...
- BF16（Brain Float 16）有 8 位指数位，与 FP32 的指数范围同量级，因此
  梯度因范围不足而下溢的风险通常低于 FP16。
- BF16 的尾数位只有 7 位（FP16 为 10 位），舍入误差更大；算子支持、溢出和训练稳定性仍需监控。
- **结论**：BF16 通常不使用 GradScaler，但没有“全面取代 FP16”的通用结论，
  应按硬件吞吐、算子覆盖与收敛质量选择。
...
- **优势 1**：更早回收已完成 backward 的激活；峰值同时存活的 microbatch
  数量依 stage 位置、warmup、实现与模型切分而定，不能写成固定 2 倍。
- **优势 2**：更早释放已完成反向传播的层的激活值。
- **Bubble**：对基础 flush 版本，其空泡开销量级与相同 $p,m$ 的 GPipe
  相近；interleaving、virtual stages 等会改变结果。
...
不能只由“175B”推出固定的 GPU 数或 TP/PP/DP。面试时应先说明输入并给出
可验证的设计流程：

1. 明确训练 token 数、层数/隐藏维度、序列长度、全局 batch、精度与优化器，
   以及目标墙钟时间、容错要求和硬件拓扑。
2. 分别估算持久化模型状态、激活、临时 buffer 和通信内存，并用小规模
   profile 校准估算。按本章的 FP16/BF16 + Adam 假设，仅持久化状态为
   $16\times175\text{B}\approx2.8\text{TB}$，激活与临时峰值另计。
3. 先把通信频繁的 TP 映射到高带宽域；再按层容量和空泡选择 PP；最后根据
   全局 batch 与收敛约束选择 DP，并决定是否使用 ZeRO/FSDP、序列/上下文并行。
4. 用 collective microbenchmark、tokens/s、MFU、峰值显存、step-time 分解和
   checkpoint 恢复演练验收。`TP=8, PP=16, DP=8` 只能作为 1024 rank 的算术
   示例，不能称为 175B 的推荐或最低方案。
...
**显存与计算的权衡**：
- 激活显存下降多少取决于 checkpoint 的边界、保存输入、不可重算状态和框架实现。
- 额外计算取决于被重算的算子与次数；必须在目标模型、序列长度和调度上 profile，
  不能用固定百分比或一组脱离实现的 $O(\sqrt L)$ 公式替代测量。

**使用场景**：
- 当激活峰值限制可用 microbatch/序列长度，且可接受重算开销时评估。
- 可以与 ZeRO/FSDP 等模型状态分片组合；二者节省的是不同内存组成。
...
**Ring AllReduce（环形算法，简化模型）**：
- **算法**：GPU 排列成环形，数据沿环传递。
- **步骤数**：$2(N-1)$ 步（ReduceScatter + AllGather 各 $N-1$ 步）。
- **带宽利用率**：对大消息通常有较好带宽效率，实际仍受拓扑与协议影响。
- **延迟**：与 $N$ 成正比。
- **适用拓扑**：可用于多种拓扑，不能只按“有无 NVSwitch”判断。

**Tree AllReduce（树形算法，简化模型）**：
- **算法**：GPU 排列成树形（通常是双层二叉树），数据从叶子汇集到根再广播。
- **步骤数**：$2\log_2(N)$ 步。
- **带宽利用率**：与 tree 结构、通道数、消息大小和拓扑有关；不能统一判定低于 Ring。
- **延迟**：与 $\log_2(N)$ 成正比。
- **适用场景**：常用于降低小消息/大规模 rank 的延迟，但仍由 NCCL 实现与拓扑决定。

**选择规则**：
- 小消息可优先观察 Tree，较大消息可优先观察 Ring，但这是起点而非定律。
- NCCL 默认会自动选择；异常拓扑或专项调优时仍应结合 `NCCL_DEBUG` 与基准验证，
  不应把“自动”解释为任何环境都达到全局最优。
...
| **与 HF 生态集成** | Transformers/Accelerate 提供集成 | Transformers/Accelerate 提供集成 |
| **配置侧重点** | sharding strategy、mesh 与模块粒度 | ZeRO stage、bucket、offload 与运行时配置 |
| **生态与性能** | PyTorch 原生组合能力强 | 提供 ZeRO、offload 与运行时生态 |
| **共同选择依据** | 目标模型支持、状态保存/恢复与 PyTorch 组合能力 | offload、混合并行、运维复杂度与目标集群实测 |

**FSDP2**：
- 基于 `DTensor` 的新实现，统一了张量并行和 FSDP 的底层表示。
- 支持更灵活的混合并行策略。
- API、checkpoint 互操作与性能特征仍应按所用 PyTorch 版本验证，不能承诺必然更快。

> **面试建议**：如果面试官问“选 FSDP 还是 DeepSpeed”，先列出必须功能，
> 再用同一模型/精度/batch 的显存、吞吐、恢复时间和维护成本对比；不要把
> “PyTorch 就选 FSDP、RLHF 就选 DeepSpeed”当成固定答案。
...
| 需求 | 候选策略 | 关键验收条件 |
|------|---------|------------|
| 模型状态超过单卡 | ZeRO/FSDP 分片，必要时 TP/PP | 峰值显存、吞吐、checkpoint 恢复 |
| 激活成为瓶颈 | activation checkpoint、序列/上下文并行 | 重算开销、数值一致性、通信占比 |
| 多模型 RLHF | 组件分片/卸载/量化/分阶段或解耦部署 | rollout 吞吐、峰值显存、版本一致性 |
| 大规模预训练 | TP+PP+DP/分片的混合并行 | tokens/s、MFU、扩展效率、故障恢复 |
| 长序列训练 | 序列/上下文并行 + 状态分片 | 注意力内存、负载均衡、端到端 step time |

> GPU 数不能仅由“7B/70B/175B”确定；至少还需要序列长度、全局与 micro
> batch、精度、优化器、目标时长、硬件型号/互联以及是否训练全部参数。
...
- [[11_深度学习与PyTorch]]：提供本章依赖的前置概念。
- [[12_Transformer与大模型原理]]：提供本章依赖的前置概念。
- [[16_模型微调与推理优化]]：提供本章依赖的前置概念。
- [[25_推理引擎与高性能服务]]：承接本章方法并进入下一层应用或工程问题。
- [[36_JAX与TPU大规模预训练]]：承接本章方法并进入下一层应用或工程问题。
