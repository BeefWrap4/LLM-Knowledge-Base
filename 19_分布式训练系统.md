...
| 概念 | 关键点 |
|------|--------|
| **DDP** | 多进程对等通信、Ring AllReduce 同步梯度、bucket 机制实现通信与计算 overlap、取代单线程 DP |
| **FSDP / FSDP2** | PyTorch 原生 ZeRO-3 等价、参数分片+动态 AllGather、FSDP2 引入 DTensor 统一张量表示 |
| **ZeRO Stage 1/2/3** | 依次分片 Optimizer / Optimizer+Grad / Optimizer+Grad+Param，Stage-3 在 8 卡节省 ~8x 显存 |
| **DeepSpeed** | 微软 ZeRO 生态、JSON 配置驱动、ZeRO-Offload/Infinity 卸载到 CPU/NVMe、Chat 支持 RLHF |
| **Megatron-LM (3D 并行)** | NVIDIA TP+PP+DP 组合、TP≤8 节点内 NVLink、典型配置 TP=8×PP=16×DP=8 训练 175B+ |
| **张量并行 (TP)** | 单层权重按列/行切分、每层 2 次 AllReduce、仅限节点内；Attention 按 head 切分最自然 |
| **流水线并行 (PP)** | 按层切分到不同卡、Bubble 比例=(p-1)/m、1F1B 调度将激活值缓存从 2m 降至 p |
| **混合精度 (FP16/BF16/FP8)** | BF16 指数位 8 位无需 Loss Scaling、FP8 配合 Transformer Engine 2.0 在 H100/B200 训练 |
| **NCCL 通信原语** | AllReduce / AllGather / ReduceScatter / Broadcast，Ring 算法带宽最优、Tree 算法延迟最低 |
