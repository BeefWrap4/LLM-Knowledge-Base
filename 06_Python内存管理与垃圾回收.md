...
tags:
  ...
  - 分代回收
---
# 第 6 章 Python 内存管理与垃圾回收 ⭐⭐⭐⭐

> [!abstract] 本章导航
> **定位**：解释 Python 内存与生命周期，为性能诊断和长时间服务稳定性打基础。
>
> **先修**：[[01_Python编程基础]]、[[02_Python核心面试专题_可变性与拷贝]]。
>
> **学习目标**：
...
>
> **建议路径**：内存管理机制 → 垃圾回收机制 → 内存泄漏与调试。先完成主线，再按需要阅读进阶内容。
>
> **配套代码**：`code/ch06_memory_gc/`。
...
1. 你能否解释引用计数、循环垃圾回收和对象分配机制？
2. 你能否使用可复现方法定位内存增长与对象滞留？
3. 你能否根据证据选择 profiling、弱引用或生命周期修复方案？

## 🧪 配套代码与验收

配套目录：`code/ch06_memory_gc/`。从 `code/` 目录运行：

```powershell
python scripts/run_all_examples.py --tier core --chapter ch06 --parallel 1 --timeout 60
```
